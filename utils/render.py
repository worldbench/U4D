...
def make_Rt(
    ...
    device: Union[torch.device,
                  str] = 'cpu') -> Tuple[torch.Tensor, torch.Tensor]:
...
def render_point_clouds(points: torch.Tensor,
                        colors: torch.Tensor | None = None,
                        size: int = 800,
                        R: torch.Tensor | None = None,
                        t: torch.Tensor | None = None,
                        focal_length=1.0) -> torch.Tensor:
    points = points.clone()
    points[..., 2] *= -1
...
def bilinear_rasterizer(coords: torch.Tensor, values: torch.Tensor,
                        out_shape: Tuple[int, int]) -> torch.Tensor:
    """
...
def estimate_surface_normal(points: torch.Tensor,
                            d: int = 2,
                            mode: str = 'closest') -> torch.Tensor:
...
@torch.no_grad()
def colorize(tensor: torch.Tensor,
             cmap_fn: plt.colors.ListedColormap = cm.turbo) -> torch.Tensor:
