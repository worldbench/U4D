...
class Resample(nn.Module):
    ...
    def __init__(self,
                 up: int = 1,
                 down: int = 1,
                 window: Tuple[int, int, int, int] = (1, 3, 3, 1),
                 ring: bool = True,
                 normalize: bool = True,
                 direction: str = 'hw',
                 mode: str = 'constant'):
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        ...
        # up by zero-intertion
        B, C, H, W = h.shape
        h = h.view(B, C, H, 1, W, 1)
        h = F.pad(h, [0, self.up_w - 1, 0, 0, 0, self.up_h - 1])
        h = h.view(B, C, H * self.up_h, W * self.up_w)
        # crop
        h = h[..., self.margin * self.up_h -
              self.ph0:(H - self.margin) * self.up_h + self.ph1,
              self.margin * self.up_w -
              self.pw0:(W - self.margin) * self.up_w + self.pw1, ]
        # fir
